...
class BihorTestRunner
{
    ...
    private function loadBihorData(): void
    {
        $response = $this->makeRequest('/ROMANIA/BIHOR/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadBihorData', "Failed to load BIHOR data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->BIHORData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadBihorData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadBihorData', 'BIHOR data loaded successfully');
    }

    private function findInBihorData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->BIHORData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->BIHORData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->BIHORData['comuna'] ?? [] as $c) {
    ...
    public function testBihorInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBihorInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBihorInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'BIHOR')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testBihorInJudeteEndpoint', 
            $found ? 'BIHOR found in judete endpoint' : 'BIHOR not found in judete endpoint');
    }

    public function testBihorEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/BIHOR/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBihorEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBihorEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'BIHOR')) {
            $this->log('FAIL', 'testBihorEndpointAccessible', "Expected BIHOR, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testBihorEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInBihorData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in BIHOR data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInBihorData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in BIHOR data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInBihorData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in BIHOR data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInBihorData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in BIHOR data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=BIHOR");
...
<head>
    ...
    <title>BIHOR County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ BIHOR County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by BihorTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  BIHOR County API Test Suite\n";
        ...
        echo "\nLoading BIHOR data from API...\n";
        $this->loadBihorData();
        
        echo "\nTesting endpoints...\n";
        $this->testBihorInJudeteEndpoint();
        $this->testBihorEndpointAccessible();

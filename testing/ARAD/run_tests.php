...
class AradTestRunner
{
    ...
    private function loadAradData(): void
    {
        $response = $this->makeRequest('/ROMANIA/ARAD/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadAradData', "Failed to load ARAD data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->aradData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadAradData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadAradData', 'ARAD data loaded successfully');
    }

    private function findInAradData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->aradData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->aradData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->aradData['comuna'] ?? [] as $c) {
    ...
    public function testAradInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testAradInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testAradInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'ARAD')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testAradInJudeteEndpoint', 
            $found ? 'ARAD found in judete endpoint' : 'ARAD not found in judete endpoint');
    }

    public function testAradEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/ARAD/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testAradEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testAradEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'ARAD')) {
            $this->log('FAIL', 'testAradEndpointAccessible', "Expected ARAD, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testAradEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInAradData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in ARAD data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInAradData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in ARAD data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInAradData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in ARAD data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInAradData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in ARAD data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=ARAD");
...
<head>
    ...
    <title>ARAD County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ ARAD County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by AradTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  ARAD County API Test Suite\n";
        ...
        echo "\nLoading ARAD data from API...\n";
        $this->loadAradData();
        
        echo "\nTesting endpoints...\n";
        $this->testAradInJudeteEndpoint();
        $this->testAradEndpointAccessible();

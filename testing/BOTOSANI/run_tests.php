...
class BotosaniTestRunner
{
    ...
    private function loadBotosaniData(): void
    {
        $response = $this->makeRequest('/ROMANIA/BOTOSANI/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadBotosaniData', "Failed to load BOTOSANI data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->botosaniData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadBotosaniData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadBotosaniData', 'BOTOSANI data loaded successfully');
    }

    private function findInBotosaniData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->botosaniData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->botosaniData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->botosaniData['comuna'] ?? [] as $c) {
    ...
    public function testBotosaniInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBotosaniInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBotosaniInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'BOTOSANI')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testBotosaniInJudeteEndpoint', 
            $found ? 'BOTOSANI found in judete endpoint' : 'BOTOSANI not found in judete endpoint');
    }

    public function testBotosaniEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/BOTOSANI/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBotosaniEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBotosaniEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'BOTOSANI')) {
            $this->log('FAIL', 'testBotosaniEndpointAccessible', "Expected BOTOSANI, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testBotosaniEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInBotosaniData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in BOTOSANI data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInBotosaniData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in BOTOSANI data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInBotosaniData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in BOTOSANI data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInBotosaniData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in BOTOSANI data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=BOTOSANI");
...
<head>
    ...
    <title>BOTOSANI County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ BOTOȘANI County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by BotosaniTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  BOTOȘANI County API Test Suite\n";
        ...
        echo "\nLoading BOTOSANI data from API...\n";
        $this->loadBotosaniData();
        
        echo "\nTesting endpoints...\n";
        $this->testBotosaniInJudeteEndpoint();
        $this->testBotosaniEndpointAccessible();

...
class BuzauTestRunner
{
    ...
    private function loadBuzauData(): void
    {
        $response = $this->makeRequest('/ROMANIA/BUZAU/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadBuzauData', "Failed to load BUZAU data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->buzauData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadBuzauData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadBuzauData', 'BUZAU data loaded successfully');
    }

    private function findInBuzauData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->buzauData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->buzauData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->buzauData['comuna'] ?? [] as $c) {
    ...
    public function testBuzauInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBuzauInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBuzauInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'BUZAU')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testBuzauInJudeteEndpoint', 
            $found ? 'BUZAU found in judete endpoint' : 'BUZAU not found in judete endpoint');
    }

    public function testBuzauEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/BUZAU/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBuzauEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBuzauEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'BUZAU')) {
            $this->log('FAIL', 'testBuzauEndpointAccessible', "Expected BUZAU, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testBuzauEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInBuzauData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in BUZAU data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInBuzauData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in BUZAU data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInBuzauData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in BUZAU data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInBuzauData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in BUZAU data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=BUZAU");
...
<head>
    ...
    <title>BUZAU County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ BUZAU County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by BuzauTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  BUZAU County API Test Suite\n";
        ...
        echo "\nLoading BUZAU data from API...\n";
        $this->loadBuzauData();
        
        echo "\nTesting endpoints...\n";
        $this->testBuzauInJudeteEndpoint();
        $this->testBuzauEndpointAccessible();

...
class ArgesTestRunner
{
    ...
    private function loadArgesData(): void
    {
        $response = $this->makeRequest('/ROMANIA/ARGES/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadArgesData', "Failed to load ARGES data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->argesData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadArgesData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadArgesData', 'ARGES data loaded successfully');
    }

    private function findInArgesData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->argesData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->argesData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->argesData['comuna'] ?? [] as $c) {
    ...
    public function testArgesInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testArgesInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testArgesInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'ARGES')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testArgesInJudeteEndpoint', 
            $found ? 'ARGES found in judete endpoint' : 'ARGES not found in judete endpoint');
    }

    public function testArgesEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/ARGES/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testArgesEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testArgesEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'ARGES')) {
            $this->log('FAIL', 'testArgesEndpointAccessible', "Expected ARGES, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testArgesEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInArgesData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in ARGES data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInArgesData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in ARGES data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInArgesData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in ARGES data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInArgesData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in ARGES data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=ARGES");
...
<head>
    ...
    <title>ARGES County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ ARGES County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by ArgesTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  ARGES County API Test Suite\n";
        ...
        echo "\nLoading ARGES data from API...\n";
        $this->loadArgesData();
        
        echo "\nTesting endpoints...\n";
        $this->testArgesInJudeteEndpoint();
        $this->testArgesEndpointAccessible();

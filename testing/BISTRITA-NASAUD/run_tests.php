...
class BistritaNasaudTestRunner
{
    ...
    private array $BISTRITA-NASAUDData = [];
    ...
    private function loadBistritaNasaudData(): void
    {
        $response = $this->makeRequest('/ROMANIA/BISTRITA-NASAUD/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'loadBistritaNasaudData', "Failed to load BISTRITA-NASAUD data: HTTP {$response['httpCode']}");
            return;
        }
        
        $this->BISTRITA-NASAUDData = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'loadBistritaNasaudData', 'Invalid JSON response');
            return;
        }
        
        $this->log('PASS', 'loadBistritaNasaudData', 'BISTRITA-NASAUD data loaded successfully');
    }

    private function findInBistritaNasaudData(string $name, string $type): ?array
    {
        ...
        switch ($type) {
            case 'municipiu':
                foreach ($this->BISTRITA-NASAUDData['municipiu'] ?? [] as $m) {
            ...
            case 'oras':
                foreach ($this->BISTRITA-NASAUDData['oras'] ?? [] as $o) {
            ...
            case 'comuna':
                foreach ($this->BISTRITA-NASAUDData['comuna'] ?? [] as $c) {
    ...
    public function testBistritaNasaudInJudeteEndpoint(): void
    {
        $response = $this->makeRequest('/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBistritaNasaudInJudeteEndpoint', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBistritaNasaudInJudeteEndpoint', 'Invalid JSON response');
            return;
        }
        ...
        foreach ($judete as $judet) {
            if ($this->matchName($judet['nume'], 'BISTRITA-NASAUD')) {
                $found = true;
                break;
            }
        }

        $this->log($found ? 'PASS' : 'FAIL', 'testBistritaNasaudInJudeteEndpoint', 
            $found ? 'BISTRITA-NASAUD found in judete endpoint' : 'BISTRITA-NASAUD not found in judete endpoint');
    }

    public function testBistritaNasaudEndpointAccessible(): void
    {
        $response = $this->makeRequest('/ROMANIA/BISTRITA-NASAUD/');
        
        if ($response['httpCode'] !== 200) {
            $this->log('FAIL', 'testBistritaNasaudEndpointAccessible', "HTTP {$response['httpCode']}");
            return;
        }

        $data = json_decode($response['body'], true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log('FAIL', 'testBistritaNasaudEndpointAccessible', 'Invalid JSON response');
            return;
        }

        if (!$this->matchName($data['nume'], 'BISTRITA-NASAUD')) {
            $this->log('FAIL', 'testBistritaNasaudEndpointAccessible', "Expected BISTRITA-NASAUD, got {$data['nume']}");
            return;
        }

        $this->log('PASS', 'testBistritaNasaudEndpointAccessible');
    }
    ...
    public function testAllMunicipii(): void
    {
        ...
        foreach ($this->testData['municipii'] as $municipiu) {
            ...
            $found = $this->findInBistritaNasaudData($municipiu, 'municipiu');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$municipiu' not found in BISTRITA-NASAUD data");
                $failed++;
                continue;
            }
    ...
    public function testAllOrase(): void
    {
        ...
        foreach ($this->testData['orase'] as $oras) {
            ...
            $found = $this->findInBistritaNasaudData($oras, 'oras');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$oras' not found in BISTRITA-NASAUD data");
                $failed++;
                continue;
            }
    ...
    public function testAllComune(): void
    {
        ...
        foreach ($this->testData['comune'] as $comuna) {
            ...
            $found = $this->findInBistritaNasaudData($comuna, 'comuna');
            
            if ($found === null) {
                $this->log('FAIL', $testName, "'$comuna' not found in BISTRITA-NASAUD data");
                $failed++;
                continue;
            }
    ...
    public function testAllSate(): void
    {
        ...
        foreach ($this->testData['sate'] as $comunaName => $sateList) {
            foreach ($sateList as $sat) {
                ...
                $found = $this->findInBistritaNasaudData($sat, 'sat');
                
                if ($found === null) {
                    $this->log('FAIL', $testName, "'$sat' not found in BISTRITA-NASAUD data");
                    $failed++;
                    continue;
                }
    ...
    private function searchForLocality(string $name): bool
    {
        ...
        $response = $this->makeRequest("/search/?q=$encodedQuery&judet=BISTRITA-NASAUD");
...
<head>
    ...
    <title>BISTRITA-NASAUD County Test Report - {$this->runId}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏛️ BISTRITA-NASAUD County Test Report</h1>
...
SECTION;
        ...
        $failedSection
        <div class="footer">
            Generated by BistritaNasaudTestRunner | Oras API Testing | Total localities tested: {$this->countTestData('all')}
        </div>
    </div>
</body>
</html>
FOOTER;
    ...
    {
        echo "========================================\n";
        echo "  BISTRITA-NASAUD County API Test Suite\n";
        ...
        echo "\nLoading BISTRITA-NASAUD data from API...\n";
        $this->loadBistritaNasaudData();
        
        echo "\nTesting endpoints...\n";
        $this->testBistritaNasaudInJudeteEndpoint();
        $this->testBistritaNasaudEndpointAccessible();
...
$runner = new BistritaNasaudTestRunner();
$runner->run();
